...
public class ImageSize {
	private static class Observer implements ImageObserver {
		MutableInt w, h;
		boolean error = false;
		Observer(MutableInt width,MutableInt height) {
			w = width;
			h = height;
		}
		public boolean imageUpdate(Image img, int infoflags, int x, int y,
			...
			synchronized(this) {
				...
				w.setValue(Math.max((Integer)w.getValue(), x+width));
				h.setValue(Math.max((Integer)h.getValue(), y+height));
				notify();
			}
			return false;
		}
		...
		public void load() {
			while(true) {
				synchronized(this) {
					...
					if((Integer)w.getValue()>0 && (Integer)h.getValue()>0)
						return;
					try {
						wait();
	...
	public static Dimension get(Image image) {
		...
		return new Dimension((Integer)observer.w.getValue(), (Integer)observer.h.getValue());
	}
